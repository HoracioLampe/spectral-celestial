...
async function exportToExcel(batchId) {
    try {
        // Use provided batchId or try to get from global variable
        if (!batchId) {
            batchId = window.currentBatchId;
        }

        // If still not found, try alternative sources
        if (!batchId) {
            // Try to get from batch detail display
            const batchIdDisplay = document.getElementById('detailBatchIdDisplay');
            if (batchIdDisplay) {
                const text = batchIdDisplay.textContent;
                const match = text.match(/ID de Lote:\s*(\d+)/);
                if (match) {
                    batchId = match[1];
                }
            }
        ...
        if (!batchId) {
            alert('❌ No se pudo detectar el ID del batch. Por favor, abre un batch primero.');
            return;
        }

        console.log(`[Excel Export] Using batch ID: ${batchId}`);
