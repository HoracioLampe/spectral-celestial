...
window.openSendPolModal = async () => {
    ...
    // Update available balance (reserve 2x gas)
    const gasReserve = 0.01; // Approx 2x gas for safety
...
document.getElementById('btnConfirmSendPol')?.addEventListener('click', async () => {
    ...
    if (amount > maxAvailable) {
        statusEl.innerHTML = `<div style="color: #ef4444;">❌ Cantidad excede el máximo disponible (${maxAvailable.toFixed(4)} POL)</div>`;
        statusEl.style.display = 'block';
        return;
    }
        ...
        if (data.success) {
            statusEl.innerHTML = `
                <div style="color: #10b981;">
                    ...
                    <small>Gas usado: ${parseFloat(data.gasUsed).toFixed(6)} POL</small>
                </div>
            `;
