const API_TRANSACTIONS = '/api/transactions'; // v3.2.0-Hardened-Release
...
function startFaucetPolling() {
    if (!faucetInterval) {
        // Use REFRESH_FRONTEND_SECONDS from config, default to 15s
        const intervalSeconds = parseInt(APP_CONFIG.REFRESH_FRONTEND_SECONDS) || 15;
        const intervalMs = intervalSeconds * 1000;

        console.log(`[Polling] Starting Faucet monitor every ${intervalSeconds}s`);
        faucetInterval = setInterval(checkFaucetStatus, intervalMs);
    }
}
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (btnEnterApp) {
        ...
        const newBtn = btnEnterApp.cloneNode(true);
        btnEnterApp.parentNode.replaceChild(newBtn, btnEnterApp);
        ...
        console.error("❌ Login button (btnEnterApp) NOT FOUND in DOM!");
    }
...
function attachEventListeners() {
    ...
    // if (btnExecuteBatch) btnExecuteBatch.addEventListener('click', executeBatch);

    // Removed manual "Repair Metrics" button - now automatic on batch completion
    const btnDisconnect = document.getElementById('btnDisconnect');
...
async function connectWallet() {
    ...
    if (!window.ethereum) {
        ...
        return alert("⚠️ Instala MetaMask o conecta tu Ledger a través de MetaMask");
    }
    ...
    try {
        ...
        console.log("📞 Requesting MetaMask accounts (this should open MetaMask)...");
...
async function fetchBalances() {
    ...
    try {
        ...
        // --- NEW: Trigger real-time balance check if a batch is open ---
        if (currentBatchData && typeof checkBatchBalance === 'function') {
            checkBatchBalance(currentBatchData);
        }
...
let currentBatchData = null; // Stores current batch object for real-time balance checks
...
window.showBatchList = function () {
    ...
    currentBatchId = null;
    currentBatchData = null; // Clear to stop real-time balance checks
    stopTxPolling();
...
function renderBatchFilters() {
    ...
    <div class="filter-bar">
        ...
        <div class="filter-group" style="flex: 2;">
            <label>🔍 Buscar (# Núm Lote, Desc, Detalle)</label>
        ...
        <div class="filter-group" style="justify-content: flex-end;">
            <label>&nbsp;</label>
            <button class="btn-glass btn-sm" onclick="clearBatchFilters()">Limpiar 🧹</button>
        </div>
    </div>
    `;
...
function renderBatchesList(batches) {
    ...
    batches.forEach(b => {
        ...
        tr.innerHTML = `
            <td style="font-weight: bold; color: #60a5fa;">${b.funder_batch_number || '-'}</td>
            <td style="opacity: 0.9;">${b.batch_number}</td>
            ...
            <td style="color:#fbbf24; font-weight: bold;">${b.total_gas_used ? parseFloat(b.total_gas_used).toFixed(6) + ' POL' : '-'}</td>
            <td>${progress}</td>
            <td>
...
window.openBatchDetail = async function (id) {
    ...
    try {
        ...
        if (data.batch) {
            updateDetailView(data.batch, data.stats); // Pass stats to detect resumable state

            // Auto-start polling if the batch is currently active or failed (to see progress)
            if (data.batch.status === 'SENT' || data.batch.status === 'PROCESSING' || data.batch.status === 'FAILED') {
                console.log(`[UI] Auto-starting polling for active/incomplete batch ${id}`);
                startTxPolling(id);
                // Also start the progress polling (gauge, relayer balances, etc)
                if (typeof pollBatchProgress === 'function') {
...
function updateDetailView(batch, stats = null) {
    // Store globally for real-time polling updates
    ...
    if (detailBatchTitle) {
        detailBatchTitle.textContent = `#${batch.funder_batch_number || b?.funder_batch_number || '-'} - ${batch.batch_number} - ${batch.detail}`;
    }
    if (detailBatchDesc) detailBatchDesc.textContent = batch.description || "Sin descripción";
    if (detailBatchIdDisplay) detailBatchIdDisplay.textContent = `Núm Lote: #${batch.funder_batch_number || '-'}`;
    ...
    if (batchStatsContainer) {
        ...
        const detailGasSpent = document.getElementById('detailGasSpent');
        if (detailGasSpent) {
            detailGasSpent.textContent = batch.total_gas_used ? `${parseFloat(batch.total_gas_used).toFixed(6)} POL` : '-';
        }
    }
    // Show/Hide Upload Area
    if (detailUploadContainer) {
        if (batch.status === 'PREPARING') {
            detailUploadContainer.classList.remove('hidden');
            if (uploadStatus) uploadStatus.textContent = '';
            // Show Clone button if it exists
            const btnClone = document.getElementById('btnOpenCloneModal');
            if (btnClone) btnClone.classList.remove('hidden');
    ...
    // Show/Hide Payment Trigger (Step 2)
    const paymentTriggerZone = document.getElementById('paymentTriggerZone');
    const btnExecute = document.getElementById('btnExecuteBatch');
    const pendingCount = parseInt(batch.pending_count) || 0;

    // A batch is "Resumable" if it's not completed but has pending TXs
    const isResumable = (batch.status === 'SENT' || batch.status === 'PROCESSING' || batch.status === 'FAILED') && pendingCount > 0;
    ...
    if (isFullyCompleted) {
        // Batch is 100% complete - show completion badge
        if (paymentTriggerZone) {
            paymentTriggerZone.innerHTML = `
                    <div style="text-align: center; padding: 1rem; background: linear-gradient(135deg, rgba(5, 150, 105, 0.1) 0%, rgba(16, 185, 129, 0.1) 100%); border: 2px solid #059669; border-radius: 12px;">
                        <div style="font-size: 1.5rem; font-weight: bold; color: #10b981; margin-bottom: 0.5rem;">✅ Lote Completado</div>
                        <div style="font-size: 0.9rem; color: #6ee7b7;">Todas las transacciones fueron procesadas exitosamente</div>
                    </div>
                `;
            paymentTriggerZone.classList.remove('hidden');
        }
    } else if (isResumable) {
        if (paymentTriggerZone) paymentTriggerZone.classList.remove('hidden');
        if (btnExecute) {
            btnExecute.textContent = "Continuar Distribución 🔄";
            btnExecute.classList.remove('btn-primary');
            btnExecute.classList.add('btn-warning');
            // Removed explicit btnExecute.disabled = false to let checkBatchBalance decide
        }
    } else if (batch.status === 'SENT' || batch.status === 'COMPLETED' || batch.status === 'PROCESSING' || batch.status === 'FAILED') {
        // Already finished or fully in-flight without being resumeable (e.g. status PROCESSING but pending is 0)
        if (paymentTriggerZone) paymentTriggerZone.classList.add('hidden');
    } else {
        // Likely READY or PREPARING
        if (paymentTriggerZone) paymentTriggerZone.classList.remove('hidden');
    }
    ...
    // --- NEW: Perform Balance Check against Funder ---
    if (typeof checkBatchBalance === 'function') {
        checkBatchBalance(batch);
    }
}
        ...
        try {
            ...
            if (data.batch) {
                ...
                if (data.batch.status !== 'SENT' && data.batch.status !== 'PROCESSING') {
                    ...
                    const total = parseInt(data.batch.total_transactions) || 0;
                    const completed = parseInt(data.batch.completed_count) || 0;
                    if (completed === total && total > 0) {
                        // All completed - replace button with completion badge
                        const paymentTriggerZone = document.getElementById('paymentTriggerZone');
                        if (paymentTriggerZone) {
                            paymentTriggerZone.innerHTML = `
                                <div style="text-align: center; padding: 1rem; background: linear-gradient(135deg, rgba(5, 150, 105, 0.1) 0%, rgba(16, 185, 129, 0.1) 100%); border: 2px solid #059669; border-radius: 12px;">
        ...
        tr.innerHTML = `
                ...
                <td class="addr-col">
                    <div class="hash-wrapper">
                        <span class="hash-link" onclick="window.open('https://polygonscan.com/tx/${tx.tx_hash}', '_blank')">
                            ${tx.tx_hash ? tx.tx_hash.substring(0, 10) + '...' : '-'}
                        </span>
                        ${tx.tx_hash ? `<button onclick="copyToClipboard('${tx.tx_hash}')" title="Copiar Hash" style="background:none; border:none; cursor:pointer; font-size:1rem;">📋</button>` : ''}
                    </div>
                    ${tx.gas_cost ? `<div style="font-size: 0.65rem; color: #fbbf24; margin-top: 2px;">⛽ ${parseFloat(tx.gas_cost).toFixed(6)} POL</div>` : ''}
                </td>
...
window.openSendPolModal = async () => {
    ...
    const polAvailableSpan = document.getElementById('polAvailableBalance');
    if (polAvailableSpan) {
        polAvailableSpan.textContent = maxAvailable.toFixed(4);
        polAvailableSpan.dataset.rawBalance = balance;
        polAvailableSpan.dataset.gasReserve = gasReserve;
        polAvailableSpan.dataset.maxSafe = maxAvailable;
    }
    ...
    if (amountInput) {
        amountInput.oninput = (e) => {
            ...
            if (e.target.value !== value) {
                ...
                if (cursor !== null) {
                    const newOffset = value.length - (e.target.value.length - cursor);
    ...
    // Clear inputs (reuse variables already declared above)
    const recipientInput = document.getElementById('polRecipientAddress');
    // amountInput already declared above at line 2800
    if (recipientInput) recipientInput.value = '';
    if (amountInput) amountInput.value = '';

    modal.classList.remove('hidden');

    // Override onclick to prevent stacking
    modal.onclick = (e) => {
        if (e.target === modal) {
            closeSendPolModal();
        }
...
window.setMaxPol = () => {
    ...
    const maxSafeRaw = parseFloat(balanceSpan.dataset.maxSafe || "0");

    // Clear rounding: use server-provided maxSafe with floor to 4 decimals
    // We deduct a tiny 0.0001 just to be ultra-conservative with float precision
    const maxSafeVal = Math.max(0, maxSafeRaw - 0.0001);

    document.getElementById('polAmount').value = (Math.floor(maxSafeVal * 10000) / 10000).toFixed(4);
...
async function pollBatchProgress(batchId) {
    ...
    try {
        ...
        if (batch) {
            // Update Transactions Table (Refresh current page/filters)
            await fetchBatchTransactions(batchId);
            ...
            // Check if batch is complete (all transactions processed)
            const totalTransactions = parseInt(batch.total_transactions) || 0;
            const completedTransactions = parseInt(batch.completed_count) || 0;
            const failedTransactions = parseInt(batch.failed_count) || 0;
            const processedTransactions = completedTransactions + failedTransactions;
            ...
            if ((processedTransactions >= totalTransactions && totalTransactions > 0) || isBackendDone || timerStopped) {
                // Batch completion detected
                stopTimer();

                // Batch is complete - do final refresh to get updated gas metrics
                console.log(`[Poll] Batch ${batchId} complete, fetching final metrics...`);

                // Fetch fresh batch data to get total_gas_used
                const finalBatchRes = await authenticatedFetch(`/api/batches/${batchId}`);
                const finalData = await finalBatchRes.json();

                if (finalData.batch) {
                    // Update gas display
                    const detailGasSpent = document.getElementById('detailGasSpent');
                    if (detailGasSpent && finalData.batch.total_gas_used) {
                        detailGasSpent.textContent = `${parseFloat(finalData.batch.total_gas_used).toFixed(6)} POL`;
                    }
                }
                ...
                if (processStatus) {
                    if (failedTransactions > 0) {
                        processStatus.textContent = `⚠️ Distribución Finalizada con ${failedTransactions} errores`;
                ...
                if (window.balanceInterval) {
                    ...
                    // Use currentBatchId instead of batchId (which is only a parameter)
                    window.balanceInterval = setInterval(() => pollBatchProgress(currentBatchId), 10000);
                }
            }
        }
    } catch (err) {
...
function renderRelayerBalances(explicitData) {
    ...
    const totalBalance = data.reduce((acc, r) => acc + parseFloat(r.balance || 0), 0);
    const totalBalanceColor = totalBalance < 0.05 ? '#ef4444' : '#4ade80';
    ...
    if (infoDiv) {
        infoDiv.innerHTML = `
            ...
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 0.5rem; background: rgba(255,255,255,0.03); padding: 0.5rem 1rem; border-radius: 6px;">
                <span style="font-size: 0.85rem; color: #cbd5e1;">💰 Total POL en Swarm:</span>
                <span style="font-weight: bold; color: ${totalBalanceColor};">${totalBalance.toFixed(6)} POL</span>
            </div>
        `;
    }

    const setupBtn = document.getElementById('btnSetupRelayers');
    const triggerZone = document.getElementById('paymentTriggerZone');
    const executeBtn = document.getElementById('btnExecuteBatch');

    // If relayers have almost no gas, allow re-funding
    if (totalBalance < 0.05) {
        if (setupBtn) {
            setupBtn.classList.remove('hidden');
            setupBtn.textContent = "Re-fondear Relayers 🏗️";
            setupBtn.style.background = "linear-gradient(135deg, #f59e0b 0%, #d97706 100%)";
        }
        if (executeBtn) {
            executeBtn.disabled = true;
            executeBtn.title = "Los relayers no tienen POL suficiente.";
        }
    } else {
    ...
    const setupBtnMain = document.getElementById('btnSetupRelayers');
    const triggerZoneMain = document.getElementById('paymentTriggerZone');

    if (data.length > 0) {
        document.getElementById('executionZone')?.classList.remove('hidden');
        // Managed now by renderRelayerBalances based on balance
    } else {
        if (setupBtnMain) {
            setupBtnMain.classList.remove('hidden');
            setupBtnMain.textContent = "1. Preparar Relayers 🏗️";
            setupBtnMain.style.background = ""; // Default
        }
        if (triggerZoneMain) triggerZoneMain.classList.add('hidden');
    }

    // Auto-refresh relayers every 15s if viewing batch details
    if (window.relayerRefreshInterval) clearInterval(window.relayerRefreshInterval);
    window.relayerRefreshInterval = setInterval(() => {
        // Use currentBatchId since batchId is not defined in this scope
        if (currentBatchId) {
            fetchRelayerBalances(currentBatchId);
        } else {
            clearInterval(window.relayerRefreshInterval);
        }
    }, 15000);
}
...
// --- UI HELPERS ---
window.updateFilenameDisplay = (input) => {
    const display = document.getElementById('fileNameDisplay');
    if (display) {
        display.textContent = input.files && input.files[0] ? input.files[0].name : 'Ningún archivo seleccionado';
    }
};

/**
 * Checks if the Funder wallet has enough USDC to cover the batch total.
 * Displays a warning alert and disables the Merkle generation if balance is insufficient.
 */
async function checkBatchBalance(batch) {
    const alertEl = document.getElementById('balanceWarningAlert');
    const messageEl = document.getElementById('balanceWarningMessage');
    const btnGenerate = document.getElementById('btnGenerateMerkle');

    if (!alertEl || !messageEl || !batch) return;

    // We use the batch total from the object.
    const totalRequiredRaw = batch.total_usdc || '0';
    const totalRequired = BigInt(totalRequiredRaw);

    if (totalRequired === 0n) {
        alertEl.classList.add('hidden');
        [btnGenerate, document.getElementById('btnExecuteBatch'), document.getElementById('btnSetupRelayers')].forEach(b => {
    ...
    try {
        const funder = (userAddress || localStorage.getItem('user_address'))?.toLowerCase();
        if (!funder) {
            console.log("[BalanceCheck] Wallet not connected yet, skipping initial check.");
            return;
        }

        // Fetch current funder balance
        const response = await authenticatedFetch(`/api/balances/${funder}`);
        const data = await response.json();

        // Use formatted usdc string from backend and convert to BigInt for comparison
        const currentBalance = ethers.parseUnits(data.usdc || "0.00", 6);

        const totalRequiredFloat = parseFloat(totalRequired.toString()) / 1000000;
        const currentBalanceFloat = parseFloat(data.usdc || "0.00");

        console.log(`[BalanceCheck] Required: ${totalRequiredFloat}, Available: ${currentBalanceFloat}`);

        if (currentBalance < totalRequired) {
            alertEl.classList.remove('hidden');
            messageEl.innerHTML = `El balance actual (<strong>$${currentBalanceFloat.toFixed(6)} USDC</strong>) debe ser <strong>mayor o igual</strong> al monto total del lote (<strong>$${totalRequiredFloat.toFixed(6)} USDC</strong>). Por favor, fondee la wallet para continuar.`;

            const btnExec = document.getElementById('btnExecuteBatch');
            const btnRelay = document.getElementById('btnSetupRelayers');

            [btnGenerate, btnExec, btnRelay].forEach(btn => {
                if (btn) {
                    btn.disabled = true;
                    btn.title = 'Balance insuficiente en Funder';
        ...
        console.error('Error in checkBatchBalance:', e);
    }
}
...
let cloneSearchTimer;
window.debounceCloneSearch = function () {
    clearTimeout(cloneSearchTimer);
    cloneSearchTimer = setTimeout(() => {
        const val = document.getElementById('cloneSearchInput')?.value || '';
        fetchCloneBatches(val);
    }, 500);
};

async function fetchCloneBatches(search = '') {
    const body = document.getElementById('cloneBatchesListBody');
    if (!body) return;
    ...
    try {
        ...
        const batches = data.batches || [];
        renderCloneBatches(batches);
    } catch (err) {
        console.error("Error fetching batches for clone:", err);
        body.innerHTML = '<tr><td colspan="5" style="text-align:center; color: #ef4444;">Error al cargar lotes</td></tr>';
    }
}

function renderCloneBatches(batches) {
    const body = document.getElementById('cloneBatchesListBody');
    if (!body) return;

    body.innerHTML = '';
    if (batches.length === 0) {
        body.innerHTML = '<tr><td colspan="5" style="text-align:center; padding: 2rem; opacity: 0.6;">No se encontraron lotes anteriores.</td></tr>';
        return;
    }

    batches.forEach(b => {
        // Don't allow cloning into itself
        if (typeof currentBatchData !== 'undefined' && currentBatchData && b.id === currentBatchData.id) return;

        const tr = document.createElement('tr');
        tr.style.borderBottom = '1px solid rgba(255,255,255,0.05)';

        let totalVal = (b.total_usdc !== null && b.total_usdc !== undefined) ? parseFloat(b.total_usdc) : 0;
        const totalDisp = `$${(totalVal / 1000000).toFixed(2)}`;

        tr.innerHTML = `
            <td style="padding: 0.75rem; font-weight: bold; color: #60a5fa;">#${b.funder_batch_number || '-'}</td>
            <td style="padding: 0.75rem;">
                <div style="font-weight: 500;">${b.batch_number}</div>
                <div style="font-size: 0.75rem; opacity: 0.6;">${b.detail || ''}</div>
            </td>
            <td style="padding: 0.75rem; text-align: center;">${b.total_transactions}</td>
            <td style="padding: 0.75rem; text-align: right; color: #4ade80;">${totalDisp}</td>
            <td style="padding: 0.75rem; text-align: center;">
                <button class="btn btn-primary btn-sm" onclick="executeClone(${b.id})">Seleccionar</button>
            </td>
        `;
        body.appendChild(tr);
    });
...
window.executeClone = async function (sourceId) {
    // Use currentBatchData which is set in updateDetailView
    if (!currentBatchData || !currentBatchData.id) {
        alert("Error: No hay un lote activo seleccionado");
        return;
    }

    if (!confirm("¿Estás seguro de que quieres clonar todas las transacciones de este lote? Se sobrescribirá cualquier transacción actual en este borrador.")) {
        return;
    }

    try {
        const res = await authenticatedFetch(`/api/batches/${currentBatchData.id}/clone`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ sourceBatchId: sourceId })
        });
        const result = await res.json();

        if (result.error) throw new Error(result.error);

        alert("✅ Transacciones clonadas exitosamente");
        closeCloneModal();

        // Refresh detail view
        openBatchDetail(currentBatchData.id);
    } catch (err) {
        console.error("Clone Error:", err);
        alert("Error al clonar el lote: " + err.message);
    }
};
