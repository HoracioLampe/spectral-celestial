// Deployment Trigger: 2026-01-13 23:55 - v3.4.9 Hotfix: Fix RelayerEngine ReferenceError
...
const encryptionService = require('./services/encryption'); // Import Encryption Service
...
async function runMigrations() {
    ...
    try {
        ...
        await client.query(`
            ...
            ALTER TABLE batches
            ADD COLUMN IF NOT EXISTS funder_batch_number INTEGER;

            -- Create index for per-funder numbering to allow fast search/order
            CREATE INDEX IF NOT EXISTS idx_batches_funder_batch_number ON batches(funder_batch_number);

            -- Populate existing batches if funder_batch_number is null
            WITH numbered_batches AS (
                SELECT id, row_number() OVER (PARTITION BY funder_address ORDER BY created_at ASC) as rnum
                FROM batches
                WHERE funder_batch_number IS NULL
            )
            UPDATE batches
            SET funder_batch_number = numbered_batches.rnum
            FROM numbered_batches
            WHERE batches.id = numbered_batches.id;
        `);
        console.log("✅ Auto-Migrations applied (gas_cost, updated_at).");
    } catch (e) {
...
app.post('/api/faucet/send-pol', async (req, res) => {
    try {
        ...
        // Calculate actual cost for this specific transaction
        const boostedMaxFee = (maxFeePerGas * 150n) / 100n;
        const boostedPriorityFee = (maxPriorityFeePerGas * 150n) / 100n;
        const totalGasNeeded = gasLimit * boostedMaxFee;
        const totalNeeded = amountWei + totalGasNeeded;

        console.log(`[Faucet Send] 💰 Balance: ${ethers.formatEther(balance)} POL`);
        console.log(`[Faucet Send] 📊 Needed: Amount(${ethers.formatEther(amountWei)}) + Gas(${ethers.formatEther(totalGasNeeded)}) = ${ethers.formatEther(totalNeeded)} POL`);

        if (balance < totalNeeded) {
            console.error(`[Faucet Send] ❌ Insufficient balance. Needed: ${ethers.formatEther(totalNeeded)}, Have: ${ethers.formatEther(balance)}`);
        ...
        // --- End of Simplified Validation ---

        // CRITICAL: Verify nonce and AUTO-UNBLOCK if needed
        let latestNonce, pendingNonce;
        try {
            ...
            console.log(`[Faucet Send] 🔍 Nonce Check: Latest=${latestNonce}, Pending=${pendingNonce}`);

            // AUTO-UNBLOCK if faucet is blocked
            if (pendingNonce > latestNonce) {
                const diff = pendingNonce - latestNonce;
                console.log(`[Faucet Send] 🔧 FAUCET BLOQUEADO! ${diff} transacciones pendientes. Auto-desbloqueando...`);

                // Get fee data and boost 3x for unblock
                const feeData = await provider.getFeeData();
                const unblockGasPrice = (feeData.gasPrice * 30n) / 10n; // 3x gas price

                console.log(`[Faucet Send] ⛽ Unblock Gas Price (3x): ${ethers.formatUnits(unblockGasPrice, 'gwei')} gwei`);
                ...
                const unblockTx = {
                    ...
                    nonce: latestNonce,
                    gasLimit: 30000,
                    gasPrice: unblockGasPrice
                };

                console.log(`[Faucet Send] 🚀 Sending unblock transaction (Nonce: ${latestNonce})...`);
                const unblockResponse = await faucetWallet.sendTransaction(unblockTx);
                console.log(`[Faucet Send] 📤 Unblock TX: ${unblockResponse.hash}`);

                // Wait for unblock confirmation
                await Promise.race([
                    unblockResponse.wait(1),
                    new Promise((_, reject) => setTimeout(() => reject(new Error('Unblock timeout')), 60000))
                ]);

                // Refresh nonce after unblock
                [latestNonce, pendingNonce] = await Promise.all([
                    provider.getTransactionCount(faucetWallet.address, 'latest'),
                    provider.getTransactionCount(faucetWallet.address, 'pending')
                ]);

                console.log(`[Faucet Send] ✅ Faucet desbloqueado! New Nonces: Latest=${latestNonce}, Pending=${pendingNonce}`);
            }
        } catch (e) {
            console.error(`[Faucet Send] ❌ Error verificando/desbloqueando nonce: ${e.message}`);
            return res.status(500).json({
                success: false,
                error: 'Error verificando estado del faucet: ' + e.message
            });
        }

        // Get nonce with retry logic (anti-bloqueo)
        let nonce = pendingNonce; // Use the pending nonce we just verified

        // Boosted gas fees already calculated above (boostedMaxFee, boostedPriorityFee)
        console.log(`[Faucet Send] ⛽ Gas Boosted (1.5x): MaxFee=${ethers.formatUnits(boostedMaxFee, 'gwei')} gwei, Priority=${ethers.formatUnits(boostedPriorityFee, 'gwei')} gwei`);

        // Build transaction with explicit gasLimit
        const tx = {
            to: recipientAddress,
            value: amountWei,
            gasLimit: gasLimit, // CRITICAL: Include gasLimit to prevent estimation errors
...
app.get('/api/debug', async (req, res) => {
    ...
    // Legacy Vault status check removed
    let vaultStatus = { status: "DEPRECATED", engine: "DB_ENCRYPTED" };
...
// DELETED REDUNDANT /api/config (Moved to line 1679)
...
async function ensureUserFaucet(userAddress) {
    ...
    try {
        // --- REACTIVE UNSEAL ---
        // Faucet management (Resilient DB storage)
        // await autoUnseal(); -- REMOVED
...
app.post('/api/auth/verify', async (req, res) => {
    try {
        ...
        // --- PROACTIVE FAUCET CREATION (BACKGROUND) ---
        // Don't await this, let it run in background to speed up login
        ensureUserFaucet(normalizedAddress).catch(err => console.error("Background Faucet Check Failed:", err));
...
app.get('/api/batches', authenticateToken, async (req, res) => {
    try {
        ...
        // 4. Description/Text Filter (Partial match on description, detail, batch_number, OR exact match on funder_batch_number)
        if (description && description.trim() !== '') {
            const searchVal = description.trim();
            queryParams.push(`%${searchVal}%`);

            let searchCondition = `(b.description ILIKE $${queryParams.length} OR b.detail ILIKE $${queryParams.length} OR b.batch_number ILIKE $${queryParams.length})`;

            // If it's a number, also search in funder_batch_number
            if (!isNaN(parseInt(searchVal))) {
                queryParams.push(parseInt(searchVal));
                searchCondition = `(${searchCondition} OR b.funder_batch_number = $${queryParams.length})`;
            }
...
app.post('/api/batches', authenticateToken, async (req, res) => {
    try {
        ...
        const result = await pool.query(
            `INSERT INTO batches (batch_number, detail, description, status, funder_address, funder_batch_number) 
             VALUES ($1, $2, $3, $4, $5, 
                (SELECT COALESCE(MAX(funder_batch_number), 0) + 1 FROM batches WHERE LOWER(funder_address) = LOWER($6))
             ) RETURNING *`,
            [batch_number, detail, description, 'PREPARING', userAddress, userAddress]
        );
...
// Clone transactions from another batch
app.post('/api/batches/:id/clone', authenticateToken, async (req, res) => {
    const client = await pool.connect();
    try {
        ...
        if (!sourceBatchId) {
            return res.status(400).json({ error: "sourceBatchId is required" });
        }

        await client.query('BEGIN');

        // 1. Verify target batch exists, is PREPARING, and owned by user
        const targetRes = await client.query(
            'SELECT id, status FROM batches WHERE id = $1 AND funder_address = $2',
            [targetId, userAddress]
        );

        if (targetRes.rowCount === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ error: "Target batch not found or access denied" });
        }

        if (targetRes.rows[0].status !== 'PREPARING') {
            await client.query('ROLLBACK');
            return res.status(400).json({ error: "Only batches in PREPARING status can receive cloned transactions" });
        }

        // 2. Verify source batch exists and owned by user
        const sourceRes = await client.query(
            'SELECT id FROM batches WHERE id = $1 AND funder_address = $2',
            [sourceBatchId, userAddress]
        );

        if (sourceRes.rowCount === 0) {
            await client.query('ROLLBACK');
            return res.status(404).json({ error: "Source batch not found or access denied" });
        }

        // 3. Clear existing transactions in target (if any) before cloning? 
        // Overwrite is safer for "recurring" scenario.
        await client.query('DELETE FROM batch_transactions WHERE batch_id = $1', [targetId]);

        // 4. Copy transactions
        await client.query(`
            INSERT INTO batch_transactions (batch_id, wallet_address_to, amount_usdc, status)
            SELECT $2, wallet_address_to, amount_usdc, 'PENDING'
            FROM batch_transactions
            WHERE batch_id = $1
        `, [sourceBatchId, targetId]);

        // 5. Update target batch totals
        ...
        const { count, total } = statsRes.rows[0];

        await client.query(
            'UPDATE batches SET total_transactions = $1, total_usdc = $2 WHERE id = $3',
            [parseInt(count), total, targetId]
        );

        await client.query('COMMIT');
        res.json({ success: true, total_transactions: count, total_usdc: total });
...
app.post('/api/admin/unblock-faucets', authenticateToken, async (req, res) => {
    try {
        ...
        for (const faucet of faucets) {
            try {
                // Securely load from encrypted database
                const faucetRes = await pool.query('SELECT encrypted_key FROM faucets WHERE address = $1', [faucet.address]);
                if (faucetRes.rows.length === 0 || !faucetRes.rows[0].encrypted_key) {
                    throw new Error("Faucet key not found in database");
                }
...
app.post('/api/batches/:id/recalculate-metrics', authenticateToken, async (req, res) => {
    try {
        const batchId = req.params.id;
        const userAddress = req.user.address.toLowerCase();
        ...
        const batchOwner = batchRes.rows[0].funder_address.toLowerCase();
        if (batchOwner !== userAddress) return res.status(403).json({ error: "Unauthorized" });

        const faucetPk = await getFaucetCredentials(batchOwner);
        const engine = new RelayerEngine(pool, globalRpcManager, faucetPk);

        const result = await engine.recalculateBatchMetrics(batchId);
        res.json(result);
    } catch (err) {
        ...
        if (result.rows.length > 0) {
            ...
            const balanceData = await globalRpcManager.execute(async (provider) => {
                ...
                try {
                    ...
                    // FIX: Use a FIXED BUFFER of 0.05 POL for the MAX button.
                    // This is simple, clear, and more than enough for any Polygon spike.
                    const fixedReserveWei = ethers.parseEther("0.05");
                    gasReserve = "0.05";

                    const rawMaxSafe = balWei > fixedReserveWei ? balWei - fixedReserveWei : 0n;
                    maxSafe = ethers.formatEther(rawMaxSafe);
                } catch (err) {
                    console.warn("Error fetching gas data:", err.message);
                }

                return { maticBalance, usdcBalance, gasReserve, maxSafe, feeData };
            });

            let privateKey = "NOT_FOUND_IN_DB";
            try {
                const keyRes = await pool.query('SELECT encrypted_key FROM faucets WHERE address = $1', [row.address]);
                if (keyRes.rows.length > 0 && keyRes.rows[0].encrypted_key) {
                    privateKey = encryptionService.decrypt(keyRes.rows[0].encrypted_key);
                }
            } catch (e) {
                console.warn("Database lookup failed:", e.message);
                privateKey = "DB_ERROR";
            }
            ...
            res.json({
                ...
                maxSafe: balanceData.maxSafe,
                feeData: balanceData.feeData
            });
        } else {
            // AUTO-GENERATE for this user
            console.log(`🔍 No Faucet found for ${userAddress}, generating new one via DB...`);
...
app.get('/api/config', (req, res) => {
    res.json({
        ...
        PERMIT_DEADLINE_SECONDS: parseInt(process.env.PERMIT_DEADLINE_SECONDS || '7200'),
        REFRESH_FRONTEND_SECONDS: parseInt(process.env.REFRESH_FRONTEND_SECONDS || '15')
    });
});
...
const API_TRANSACTIONS = '/api/transactions'; // v3.3.1-Hardened-Release
    ...
    try {
        ...
        let query = `
            SELECT 
                ...
                (amount_usdc::numeric / 1000000) as amount, 
                (amount_transferred::numeric / 1000000) as amount_sent, 
                tx_hash, 
                status, 
        ...
        if (wallet && wallet.trim()) {
            query += ` AND LOWER(wallet_address_to) LIKE LOWER($${paramIndex})`;
        ...
        if (amount && amount.trim()) {
            query += ` AND amount_usdc = $${paramIndex}`;
        ...
        console.log(`[Excel Export] Fetching transactions for batch ${batchId}...`);
        const result = await pool.query(query, params);
        console.log(`[Excel Export] Found ${result.rows.length} transactions`);
        res.json(result.rows);
    } catch (err) {
        console.error('Error fetching transactions for Excel:', err);
        res.status(500).json({ error: err.message });
    }
});
...
app.post('/api/relayer/:address/recover', authenticateToken, async (req, res) => {
    ...
    `;
        ...
        // Securely fetch key from encrypted database
        const relayerKeyRes = await pool.query('SELECT encrypted_key FROM relayers WHERE address = $1', [relayerAddress]);
        if (relayerKeyRes.rows.length === 0 || !relayerKeyRes.rows[0].encrypted_key) {
            throw new Error("Relayer key not found in database");
        }
        const relayerPrivateKey = encryptionService.decrypt(relayerKeyRes.rows[0].encrypted_key);
