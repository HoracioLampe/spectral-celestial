...
/**
 * @title InstantPayment V3
 * @dev Fixes from Solidity Guardian audit (v2.0.0):
 *   - MEDIO-2: activatePolicyWithPermit emits PolicyReset when amount=0 (semantics fix)
 *   - INFO-3:  nonReentrant added to activatePolicyWithPermit and resetPolicyWithPermit
 *
 * Storage layout: No new state variables added — __gap stays at 44 slots.
 */
contract InstantPaymentV3 is
    Initializable,
    ...
    EIP712Upgradeable
{
     ...
     *
     * @dev FIX MEDIO-2 (v3): Emits PolicyReset when amount=0 (semantic clarity for indexers).
     * @dev FIX INFO-3  (v3): nonReentrant added as defense-in-depth against malicious ERC-2612 tokens.
     *
     * @param coldWallet   Address of the cold wallet (funder)
     * @param amount       USDC amount (6 decimals) — 0 allowed for atomic reset via permit
    ...
    function activatePolicyWithPermit(
        ...
        bytes32 r,
        bytes32 s
    ) external onlyRelayerOf(coldWallet) whenNotPaused nonReentrant {
        ...
        // 2. Update policy (same TX — atomic)
        policies[coldWallet] = Policy({
            initialAmount:   amount,
            consumedAmount:  0,
            deadline:        deadline,
            isActive:        amount > 0  // amount=0 resets, so isActive=false
        });

        // 3. Emit semantically correct event (FIX MEDIO-2)
        if (amount == 0) {
     ...
     *
     * @dev FIX INFO-3 (v3): nonReentrant added as defense-in-depth.
     *
        ...
        )
    {
        ...
        remaining      = usdcToken.allowance(coldWallet, address(this));
        deadline       = p.deadline;
        isActive       = p.isActive;
    ...
    // ─── Storage Gap ──────────────────────────────────────────────────────────
    // Same gap size as V1/V2 (44 slots). No new state variables added in V3.
